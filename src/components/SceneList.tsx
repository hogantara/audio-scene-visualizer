...
function statusDot(s: Scene): { cls: string; label: string } {
  switch (s.imageStatus) {
    case 'ready':
      return { cls: 'ok', label: 'Image ready' };
    case 'generating':
      return { cls: 'busy', label: 'Generating…' };
    case 'error':
      return { cls: 'err', label: 'Generation failed' };
    default:
      return { cls: 'none', label: 'No image yet' };
  }
}
